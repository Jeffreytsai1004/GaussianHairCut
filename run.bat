...
@CALL micromamba run -n gaussian_splatting_hair python preprocess_raw_images.py --data_path "%DATA_PATH%"

@REM Run COLMAP reconstruction and camera calibration
@CD "%PROJECT_DIR%\src"
@CALL micromamba run -n gaussian_splatting_hair python convert.py -s "%DATA_PATH%" --camera "OPENCV" --max_size 1024

@REM Run Matte-Anything
@CD "%PROJECT_DIR%\src\preprocessing"
@CALL micromamba run -n matte_anything python calc_masks.py --data_path "%DATA_PATH%" --image_format png --max_size 2048
...
@CD "%PROJECT_DIR%\src\preprocessing"
@CALL micromamba run -n gaussian_splatting_hair python calc_face_alignment.py --data_path "%DATA_PATH%" --image_dir "images_4"
...
@REM Delete original files to save disk space
